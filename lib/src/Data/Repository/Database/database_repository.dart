...
class DatabaseRepository extends BaseDatabaseRepository {
  ...
  Stream<List<User>> getUsers(String userId, Gender users) {
    ...
    try {
      return _firebaseFirestore.collection(users.name).snapshots().map(
  ...
  Future<List<User>> getUsersWithLimit(User user) async {
    try {
      final user2 = await _firebaseFirestore
          .collection(user.gender)
          .doc(user.id)
          .get()
          .then((doc) => User.fromSnapshoot(doc));
      final preference = await _firebaseFirestore
          .collection(user.gender)
          .doc(user.gender)
          .collection('userpreference')
          .doc('preference')
          .get()
          .then((doc) => UserPreference.fromSnapshoot(doc));
  ...
  Stream<List<UserMatch>> getMatches(String userId, Gender users) {
    try {
      return _firebaseFirestore
          ...
          .snapshots()
          .map((snap) => snap.docs
              .map((match) => UserMatch.fromSnapshoot(match))
              .toList());
    } on FirebaseException catch (e) {
      print(e.message);
      throw Exception(e.message);
  ...
  Stream<List<Like>> getLikedMeUsers(String userId, Gender users) {
    try {
      return _firebaseFirestore
          ...
          .snapshots()
          .map((snap) =>
              snap.docs.map((user) => Like.fromSnapshoot(user)).toList());
    } on FirebaseException catch (e) {
      throw Exception(e.message);
    } on Exception catch (e) {
      // TODO
  ...
  @override
  Future<void> deleteLikedMeUser(
      String userId, Gender users, String likedMeUserId) async {
    try {
      await _firebaseFirestore
          .collection(users.name)
          .doc(userId)
      ...
      var viewed = await _firebaseFirestore
          .collection(users.name)
          .doc(userId)
          .collection('viewedProfiles')
          .doc('viewed')
...
//you should come here some day and fix it
  ...
  @override
  Future<void> likeLikedMeUser(
      User user, Like likedMeUser, bool isSuperLike) async {
    try {
      await _firebaseFirestore
          .collection(user.gender)
          .doc(user.id)
  ...
  Future<void> openChat(Message message, Gender users) async {
    try {
      ...
      //for the other user
      await _firebaseFirestore
          .collection(users == Gender.men ? Gender.women.name : Gender.men.name)
          .doc(message.receiverId)
          .collection('matches')
          .doc(message.senderId)
      ...
      await _firebaseFirestore
          ...
          .collection('matches')
          .doc(message.receiverId)
          .update({'chatOpened': true}).then((value) {
        print('here');
      });
    ...
    try {
      return await _firebaseFirestore
        ...
        return await _firebaseFirestore
            ...
            .doc(userId)
            .get()
            .then((value) => User.fromSnapshoot(value));
      });
    } on FirebaseException catch (e) {
      throw Exception(e.message);
    } on Exception catch (e) {
    ...
    try {
      return await _firebaseFirestore
        ...
        return await _firebaseFirestore
            ...
            .doc(userId)
            .get()
            .then((value) => value.exists ? Gender.men : Gender.nonExist);
      });
    } on FirebaseException catch (e) {
      throw Exception(e.message);
    } on Exception catch (e) {
...
//************************ Messages repository *******************************
  ...
  Stream<List<Message>> getChats(
      String userId, Gender users, String matchedUserId) {
    try {
      return _firebaseFirestore
          .collection(users.name)
          .doc(userId)
          ...
          .snapshots()
          .map((snap) =>
              snap.docs.map((doc) => Message.fromSnapshoot(doc)).toList());
    } on FirebaseException catch (e) {
      throw Exception(e.message);
    } on Exception catch (e) {
      // TODO
  ...
  Stream<List<Message>> getLastMessage(
    ...
    try {
      var result = _firebaseFirestore
          ...
          .limit(1)
          .snapshots()
          .map((msg) => msg.docs.map((e) => Message.fromSnapshoot(e)).toList());
  ...
  Future<void> updateUserPreference(
      UserPreference userPreference, Gender users) async {
    try {
      await _firebaseFirestore
          .collection(users.name)
  ...
  @override
  Future<List<User>> getUsersBasedonNearBy(
      String userId, Gender users, int max) async {
    try {
      ...
      int totalNum = users == Gender.men
          ? remoteNumbers['howManyADayMen']
          : remoteNumbers['howManyADayWomen'];
      int remoteMax = remoteNumbers['maxKmNearBy'];

      geohash.MyGeoHash myGeoHash = geohash.MyGeoHash();
      String hash = myGeoHash.geoHashForLocation(
          geohash.GeoPoint(myLocation.latitude, myLocation.longitude));

      geohash.GeoPoint center =
          geohash.GeoPoint(myLocation.latitude, myLocation.longitude);
      //var max = prefes.maximumDistance;
      double radiusInM = remoteMax * 1000;
      // preference.maximumDistance! * 1000;
      List<List<String>> bounds =
          myGeoHash.geohashQueryBounds(center, radiusInM);
      List<Future> futures = [];

      for (List<String> b in bounds) {
        var q = _firebaseFirestore
            .collection(
                users == Gender.men ? Gender.women.name : Gender.men.name)
            .orderBy('geohash')
      ...
      var viewedProfiles = await _firebaseFirestore
          .collection(users.name)
          .doc(userId)
          .collection('viewedProfiles')
          .doc('viewed')
  ...
  Future<void> addVerifyMeUser(User user, bool onlyVerifyMe, String url) async {
    ...
    try {
      await _firebaseFirestore.collection('verify').doc(user.id).set({
        'userId': user.id,
        'gender': user.gender,
...
//   }
  ...
  @override
  Future<List<User>> getUsersMainLogic(
      String userId, Gender gender, UserPreference preference, User my) async {
    // TODO: implement getUsersMainLogic
    //try {
    var remoteNumbers = remoteConfig.getNumbers();
    int totalNum = gender == Gender.men
        ? remoteNumbers['howManyADayMen']
        : remoteNumbers['howManyADayWomen'];
    ...
    final int queenCount = await collectionRef
        .where('adminChoice',
            isEqualTo: gender == Gender.men ? 'queen' : 'king')
        .count()
        .get()
        .then((value) => value.count!);
    final int princessCount = await collectionRef
        .where('adminChoice',
            isEqualTo: gender == Gender.men ? 'princess' : 'gent')
        .count()
        .get()
        .then((value) => value.count!);
    final noOfUsers = await _firebaseFirestore
        .collection(gender == Gender.men ? Gender.women.name : Gender.men.name)
        .count()
        .get()
        .then((value) => value.count, onError: (e) => print('error counting'));

//logic for showing queens, kings, princess and gentlemen
    if (remoteNumbers['useMainLogic']) {
      viewedQueens = viewedProfiles?[gender == Gender.men ? 'queens' : 'kings']
          .split(',')
          .map(int.parse)
          .toList();
      viewedPrincess =
          viewedProfiles?[gender == Gender.men ? 'princess' : 'gents']
              .split(',')
              .map(int.parse)
              .toList();
      ...
      for (int i = 1; i <= remoteQueensNumber; i++) {
        ...
        while (viewedQueens.contains(random) ||
            randomsForQueens.contains(random)) {
      ...
      if (randomsForQueens.isNotEmpty) {
        List<User> queensOrKings = await collectionRef
            .where('adminChoice',
                isEqualTo: gender == Gender.men ? 'queen' : 'king')
            .where(gender == Gender.men ? 'queenNumber' : 'kingNumber',
                whereIn: randomsForQueens)
            .get()
            .then((snap) =>
                snap.docs.map((doc) => User.fromSnapshoot(doc)).toList());
      ...
      if (princessCount != 0) {
        for (int i = 1; i <= remoteGentsNumber; i++) {
          ...
          while (viewedPrincess.contains(random) ||
              randomsForPrincess.contains(random)) {
        ...
        randomsForPrincess
            .removeWhere((element) => viewedPrincess.contains(element));
        if (randomsForPrincess.isNotEmpty) {
          princessOrgents = await collectionRef
              .where('adminChoice',
                  isEqualTo: gender == Gender.men ? 'princess' : 'gent')
              .where(gender == Gender.women ? 'princessNumber' : 'gentNumber',
                  whereIn: randomsForPrincess)
              .get()
              .then((snap) =>
                  snap.docs.map((doc) => User.fromSnapshoot(doc)).toList());
...
//end if princess count is greater than 0
      ...
      scoreUsers.removeWhere(
        (user) => viewedMatches.contains(user.id),
      );
    ...
    if (result.length < totalNum) {
      ...
      List<User> filler = await _firebaseFirestore
          .collection(
              gender == Gender.men ? Gender.women.name : Gender.men.name)
    ...
    if (remoteNumbers['useMainLogic']) {
      if (queensOrKings.isNotEmpty) {
        String oldQueens =
            viewedProfiles?[gender == Gender.men ? 'queens' : 'kings'];
        ...
        _firebaseFirestore
            ...
            .collection('viewedProfiles')
            .doc('viewed')
            .update({gender == Gender.men ? 'queens' : 'kings': newQueens});
      }
      if (princessOrgents.isNotEmpty) {
        ...
        _firebaseFirestore
            ...
            .collection('viewedProfiles')
            .doc('viewed')
            .update({gender == Gender.men ? 'princess' : 'gents': newPrinces});
      }
    }
    ...
    if (my.countryCode == 'ET') {
      ...
      diascoraSameLookingFor
          .removeWhere((element) => viewedMatches.contains(element.id));
      ...
      if (diascoraSameLookingFor.length < remoteNumbers['numberOfDiascora']) {
        ...
        diascoraRandom
            .removeWhere((element) => viewedMatches.contains(element.id));
        diascoraRandom
            .removeWhere((element) => diascoraSameLookingFor.contains(element));
        ...
        if (diascoraSameLookingFor.length < remoteNumbers['numberOfDiascora']) {
          ...
          diascoraRandom
              .removeWhere((element) => viewedMatches.contains(element.id));
          diascoraRandom.removeWhere(
              (element) => diascoraSameLookingFor.contains(element));
          diascoraSameLookingFor.addAll(diascoraRandom);
        }
      }
  ...
  Future<bool> completeOnboarding(
    ...
    try {
      ...
      int number = await _firebaseFirestore
          .collection(user.gender)
          .count()
          .get()
          .then((value) => value.count!);
      ...
      await _firebaseFirestore
          ...
          .collection('userpreference')
          .doc('preference')
          .set(UserPreference(userId: user.id, phoneNumber: user.phoneNumber)
              .toMap());
      ...
      await _firebaseFirestore.collection(user.gender).doc(user.id).update({
        'isCompleted': true,
        'number': number,
  ...
  Future<Payment> getUserPayment(
      {required userId, required Gender users}) async {
    try {
      return await _firebaseFirestore
          .collection(users.name)
          .doc(userId)
  ...
  Future<List<User>> getUsersWithAge(String userId, Gender gender) async {
    var prefes = await _firebaseFirestore
        .collection(gender.name)
        .doc(userId)
        .collection('userpreference')
        .doc('preference')
        .get()
        .then((value) => UserPreference.fromSnapshoot(value));
    var userList = await _firebaseFirestore
        .collection(gender == Gender.men ? Gender.women.name : gender.name)
        .orderBy('age')
  ...
  Future<void> deletePhoto(
      ...
      required String userId,
      required Gender users}) async {
    try {
      await _firebaseFirestore.collection(users.name).doc(userId).update({
  ...
  Future<List<User>> getUsersBasedonPreference(
      String userId, Gender gender, UserPreference prefes, User my) async {
    CollectionReference collectionReference = _firebaseFirestore.collection(
        gender == Gender.women ? Gender.men.name : Gender.women.name);
    var remoteNumbers = remoteConfig.getNumbers();
    int totalNum = gender == Gender.men
        ? remoteNumbers['howManyADayMen']
        : remoteNumbers['howManyADayWomen'];
    ...
    int count =
        await collectionReference.count().get().then((value) => value.count!);
    ...
    Query query = collectionReference.where('age', whereIn: ageWhereIn);
    ...
    if (prefes.onlyShowOnlineMatches ?? false) {
      ...
      users = await queryRecent.limit(totalNum).get().then(
  ...
  Future<List<UserMatch>> searchMatchName(
      {required String userId,
    ...
    try {
      return await _firebaseFirestore
          ...
          .get()
          .then((value) =>
              value.docs.map((doc) => UserMatch.fromSnapshoot(doc)).toList());
    } catch (e) {
      throw Exception(e);
    }
  }
  ...
  void unMatch(
      {required String userId,
      required Gender gender,
      required UserMatch matchUser}) async {
    try {
      await _firebaseFirestore
          .collection(gender.name)
          .doc(userId)
  ...
  Stream<List<Boosted>> boostedUsers(Gender gender) {
    return _firebaseFirestore
        ...
        .snapshots()
        .map((snap) =>
            snap.docs.map((doc) => Boosted.fromSnapshoot(doc)).toList());
  }
  ...
  Future<void> updateOnlinestatus(
    ...
    try {
      await _firebaseFirestore
          .collection(gender.name)
          .doc(userId)
          .update({'online': online, 'lastseen': FieldValue.serverTimestamp()});
      var update = {
        'online': online,
        'lastseen': FieldValue.serverTimestamp(),
      };
  ...
  Stream<DocumentSnapshot<Map<String, dynamic>>> onlineStatusChanged(
      {required String userId, required String gender}) {
    try {
      return _firebaseFirestore
          .collection(gender)
          .doc(userId)
  ...
  Future<List<User>?> getOnlineUsers(
      {required String userId, required Gender gender, int? limit = 20}) async {
    try {
      ...
      List<User> recentUsers = await _firebaseFirestore
          .collection(
              gender == Gender.men ? Gender.women.name : Gender.men.name)
          .orderBy('lastseen', descending: true)
          .limit(limit!)
  ...
  reportMatch(
      {required String userId,
      required Gender gender,
      required UserMatch reportedUser,
      required int index,
      required String reportName,
      required String description}) async {
    try {
      await _firebaseFirestore.collection('report').add({
  ...
  FutureOr<User?> getRandomMatch(
      {required String userId, required Gender gender}) async {
    try {
      final noOfUsers = await _firebaseFirestore
          .collection(
              gender == Gender.men ? Gender.women.name : Gender.men.name)
          ...
          .get()
          .then((value) => value.count,
              onError: (e) => print('error counting'));
      //var random = Random().nextInt(noOfUsers!);
      final viewed = await _firebaseFirestore
          .collection(gender.name)
          .doc(userId)
          .collection('viewedProfiles')
          .doc('viewed')
      ...
      User? user = await _firebaseFirestore
          .collection(
              gender == Gender.men ? Gender.women.name : Gender.men.name)
          .where('adminChoice', isEqualTo: 'nan')
  ...
  Future<User?> getQueen(
    ...
    try {
      final viewed = await _firebaseFirestore
          ...
          .doc('viewed')
          .get();
      var viewedQueens = viewed[gender == Gender.men ? 'queens' : 'kings']
          .split(',')
          .map(int.parse)
          .toList();

      final noOfQueens = await _firebaseFirestore
          .collection(
              gender == Gender.men ? Gender.women.name : Gender.men.name)
          .where('adminChoice',
              isEqualTo: gender == Gender.men ? 'queen' : 'king')
      ...
      var queen = await _firebaseFirestore
          .collection(
              gender == Gender.men ? Gender.women.name : Gender.men.name)
          .where('adminChoice',
              isEqualTo: gender == Gender.men ? 'queen' : 'king')
  ...
  Future<User?> getPrincess(
    ...
    try {
      final viewed = await _firebaseFirestore
          ...
          .doc('viewed')
          .get();
      var viewedPrincess = viewed[gender == Gender.men ? 'princess' : 'gents']
          .split(',')
          .map(int.parse)
          .toList();

      final noOfPrince = await _firebaseFirestore
          .collection(
              gender == Gender.men ? Gender.women.name : Gender.men.name)
          .where('adminChoice',
              isEqualTo: gender == Gender.men ? 'princess' : 'gent')
      ...
      User? princess = await _firebaseFirestore
          .collection(
              gender == Gender.men ? Gender.women.name : Gender.men.name)
          .where('adminChoice',
              isEqualTo: gender == Gender.men ? 'princess' : 'gent')
          .where(gender == Gender.men ? 'princessNumber' : 'gentNumber',
              isEqualTo: random)
          .get()
          .then((value) {
      ...
      if (princess != null) {
        ...
        await _firebaseFirestore
            ...
            .collection('viewedProfiles')
            .doc('viewed')
            .update({gender == Gender.men ? 'princess' : 'gents': newPrince});
      }
  ...
  Future<User?> findMeOnHabeshaWe(
      {required String id, required Gender gender}) async {
    try {
      return await _firebaseFirestore
          .collection(
              gender == Gender.men ? Gender.women.name : Gender.men.name)
  ...
  Future<void> addToQueensKings(User user) async {
    ...
    final count = await _firebaseFirestore
        .collection(user.gender == Gender.men.name ? 'kings' : 'queens')
        .count()
        .get()
        .then((value) => value.count);
    userMap[user.gender == Gender.men.name ? 'kingNumber' : 'queenNumber'] =
        count! + 1;
    await _firebaseFirestore.collection(user.gender).doc(user.id).update({
  ...
  Future<void> addPrincessOrGent(User user) async {
    ...
    final count = await _firebaseFirestore
        .collection(user.gender)
        .where('adminChoice',
            isEqualTo: user.gender == Gender.men.name ? 'gent' : 'princess')
        .count()
        .get()
        .then((value) => value.count);
    ...
    await _firebaseFirestore.collection(user.gender).doc(user.id).update({
      ...
      user.gender == Gender.men.name ? 'gentNumber' : 'princessNumber':
          count! + 1
    });
  }
  ...
  Future<void> boostMe(User user, int after) async {
    try {
      ...
      await _firebaseFirestore
          ...
          .collection('payment')
          .doc('subscription')
          .update({'boostedTime': DateTime.now()});
  ...
  Future<List<Like>> loadMoreLikes(
      {required String userId,
      required Gender gender,
      required Timestamp startAfter}) async {
    try {
      List<Like> likes = await _firebaseFirestore
          .collection(gender.name)
          .doc(userId)
          .collection('likes')
          .orderBy('timestamp', descending: true)
  ...
  loadMoreMatches(
      ...
      required Timestamp startAfter}) async {
    try {
      List<UserMatch> matches = await _firebaseFirestore
          .collection(gender.name)
          .doc(userId)
          .collection('matches')
          .where('chatOpened', isEqualTo: false)
  ...
  void updateLocation(
      {required String userId,
      required Gender gender,
      required List<double> newLocation,
      required String hash}) async {
    try {
      await _firebaseFirestore.collection(gender.name).doc(userId).update({
  ...
  Future<List<UserMatch>> loadMoreActiveMatches(
      {required String userId,
      required Gender gender,
      required Timestamp startAfter}) async {
    try {
      var matches = await _firebaseFirestore
          .collection(gender.name)
          .doc(userId)
          .collection('matches')
          .where('chatOpened', isEqualTo: true)
  ...
  Future<int> getMatchesCount(
      {required String userId, required Gender gender}) {
    return _firebaseFirestore
        .collection(gender.name)
        .doc(userId)
        .collection('matches')
  ...
  void changeMatchImage(
      {required String userId,
      required Map<String, dynamic> match,
      required Gender userGender,
      required String from,
      List<dynamic>? newImages}) async {
    if (from == 'deleted') {
      await _firebaseFirestore
          .collection(userGender.name)
          .doc(userId)
          .collection('matches')
          .doc(match['id'])
          .update({'imageUrls': [], 'name': 'DeletedAccount'});
    } else {
      ...
      if (!listEquals(newImages, match['imageUrls'])) {
        await _firebaseFirestore
            ...
            .doc(match['id'])
            .update({
          from == 'matches' ? 'imageUrls' : 'user.imageUrls': newImages
        });
      }
    }
  }
  ...
  void checkUserExist(
      ...
      required Map<String, dynamic> match,
      required String from,
      required List newImages}) async {
    try {
      bool exist = await _firebaseFirestore
  ...
  Future<void> updateViewedFiresbase(String id, String gender) async {
    ...
    await _firebaseFirestore
        ...
        .collection('viewedProfiles')
        .doc('viewed')
        .update({'liked': '$liked${SharedPrefes.getLikedIds()}'});
    ...
    await _firebaseFirestore
        ...
        .collection('viewedProfiles')
        .doc('viewed')
        .update({'passedNumbers': newPassedNums});
  ...
  Future<String> getTelebirrImage() async {
    var telebirr =
        await _firebaseFirestore.collection('configs').doc('telebirr').get();
